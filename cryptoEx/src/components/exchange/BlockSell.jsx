...
const BlockSell = ({
  ...
  // Флаг для отслеживания пользовательского ввода
  ...
  const getCurrency = async (url, currencyTypeToFetch) => {
    try {
      ...
      if (!selectedValue && data.length > 0) {
        ...
        // Не вызываем onAmountChange здесь, чтобы избежать преждевременных расчетов
      }
    } catch (err) {
      console.error(err);
    }
  ...
  // Отдельный useEffect для уведомления родителя о изменении валюты
  useEffect(() => {
  ...
  // Основной useEffect для автоматических расчетов
  useEffect(() => {
    // Не пересчитываем, если пользователь сейчас вводит данные
    ...
    if (otherCurrencyType === 'crypto') {
      ...
      if (otherBinanceError) {
        console.error("Error getting other binance price:", otherBinanceError);
        return;
      }
      ...
      if (otherCbrError) {
        console.error("Error getting other CBR price:", otherCbrError);
        return;
      }
    ...
    if (currency === 'crypto') {
      ...
      if (ownBinanceError) {
        console.error("Error getting own binance price:", ownBinanceError);
        return;
      }
      ...
      if (ownCbrError) {
        console.error("Error getting own CBR price:", ownCbrError);
        return;
      }
      finalAmount = amountInUSD * ownCbrPrice;
    }

    if (!isNaN(finalAmount) && !isUserInput) {
      const roundedAmount = parseFloat(finalAmount.toFixed(currency === 'crypto' ? 8 : 2));
      setAmount(String(roundedAmount));
    } else if (isUserInput) {
      // Сбрасываем флаг пользовательского ввода через некоторое время
      setTimeout(() => setIsUserInput(false), 100);
    }
